...
class ViewModelsTest{
    ...
    private lateinit var checkInsTestSubscriber : TestObserver<List<CheckInPost>>
    private lateinit var beerImageTestSubscriber : TestObserver<List<Photo>>
    ...
    fun setUp(){
        ...
        beerImageTestSubscriber = TestObserver()
        gson = Gson()
    ...
    fun testGetOneCheckInPost(){
        given(untappedAPI.getCheckIns()).willReturn(Single.just(checkInRespons))
        ...
        checkInsTestSubscriber.assertValues(checkInRespons.response.checkins.items.map{it.toPost()})
    }

    @Test
    @DisplayName("Checking passing beer info from api to activity")
    fun testGetOneBeerPhoto(){
        given(untappedAPI.getBeerInfo()).willReturn(Single.just(beerInfoResponse))
        ...
        beerImageTestSubscriber.assertValues(beerInfoResponse.response.beer.medias.items.map{it.photo})
    }
