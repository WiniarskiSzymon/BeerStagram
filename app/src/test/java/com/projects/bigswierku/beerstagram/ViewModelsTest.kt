...
class ViewModelsTest{
    ...
    fun setUp(){
        ...
        beerImageTestSubscriber = TestSubscriber()
        gson = Gson()
        checkInRespons = gson.fromJson(localCheckInsJSON, PubResponse::class.java)
        beerInfoResponse = gson.fromJson(beerInfoJSON, BeerResponse::class.java)
    ...
    fun testGetOneCheckInPost(){
        given(untappedAPI.getCheckIns()).willReturn(Flowable.just(checkInRespons))
        checkInsViewModelMock.getCheckIns().subscribe(checkInsTestSubscriber)
        checkInsTestSubscriber.assertNoErrors()
        checkInsTestSubscriber.assertValueCount(1)
        checkInsTestSubscriber.assertValues(checkInRespons.checkins.items.toPost())
    }

    @Test
    @DisplayName("Checking passing beer info from api to activity")
    fun testGetOneBeerPhoto(){
        given(untappedAPI.getBeerInfo()).willReturn(Flowable.just(beerInfoResponse))
        beerImageViewModelMock.getBeerImages().subscribe(beerImageTestSubscriber)
        beerImageTestSubscriber.assertNoErrors()
        beerImageTestSubscriber.assertValueCount(1)
        beerImageTestSubscriber.assertValues(beerInfoResponse.beer.medias.items.photo)
    }
